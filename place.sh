...
echo '------------------------------------------------------------'
echo "PLACEMENT PROJECT $argv[1] TOP MODULE $argv[2] WITH GRAYWOLF"
echo '------------------------------------------------------------'
echo 

if ($#argv < 2) then
   echo "Usage:  ./place.sh <project_name> <source_name> [tech_name]"
   exit 1
else
   if ($#argv == 2) then
      set techname=osu035
...
cd ${projectpath}
source flow_var.sh
cp ${techdir}/${techname}.par ${layoutdir}/${sourcename}.par
...
graywolf ${graywolf_options} $rootname >>& ${synthlog}
