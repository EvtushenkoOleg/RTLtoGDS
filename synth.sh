#!/bin/tcsh -f
echo '---------------------------------------------------------'
echo "SYNTHESIS PROJECT $argv[1] TOP MODULE $argv[2] WITH YOSYS"
echo '---------------------------------------------------------'
echo 

if ($#argv < 2) then
   echo "Usage:  ./synth.sh <project_name> <source_name> [tech_name]"
   exit 1
else
   if ($#argv == 2) then
      set techname=osu035
...
set date=`date`
echo "Logfile created on $date" > ${synthlog}
